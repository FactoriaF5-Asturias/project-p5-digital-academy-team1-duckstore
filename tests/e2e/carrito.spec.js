...
test('añadir un patito al carrito actualiza el contador', async ({ page }) => {
    await page.goto('/index.html');
...
test('el patito aparece en el carrito tras añadirlo', async ({ page }) => {
    await page.goto('/index.html');
    await page.locator('.product-card__btn').first().click();
    await page.waitForURL('**/detalle.html**');
    await page.locator('#btn-añadir').click();
    await page.goto('/pages/carrito.html');
    const items = await page.locator('.carrito-item').count();
